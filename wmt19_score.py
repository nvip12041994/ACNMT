...
def cli_main():
    ...
    # main(args)
    cur_model = ['./data-bin/wmt17_en_de', '--beam', '5', '--path', '', '--bpe', 'subword_nmt',
                 '--bpe-codes', './data-bin/wmt17_en_de/code', '--sacrebleu', '--quiet']
    filenames = next(walk('checkpoints/transformer_wmt17/'), (None, None, []))[2]
    bleu = {}
    last_best = []
    for i in range(len(filenames)):
        parser = options.get_generation_parser()
        if "checkpoint" in filenames[i]:
            cur_model[4] = 'checkpoints/transformer_wmt17/' + filenames[i]
            args = options.parse_args_and_arch(parser, input_args=cur_model)
            result = main(args)
